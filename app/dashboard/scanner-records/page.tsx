                ...
                records.map((record) => (
                  <tr key={record.id} className="hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right whitespace-nowrap font-mono">
                      <div className={`font-bold ${record.quantity > 0 && record.actionType !== 'ORDER_OUT' ? 'text-emerald-500' : record.actionType === 'ORDER_OUT' ? 'text-red-500' : 'text-foreground/70 dark:text-foreground/70 dark:text-foreground/70 dark:text-foreground/70 dark:text-foreground/50'}`}>
